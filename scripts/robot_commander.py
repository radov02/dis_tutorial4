...
class RobotCommander(Node):
    ...
    def prefetch_interaction(self, max_attempts=2):
    ...
    def wait_for_interaction_completion(self, max_attempts=3):
        for attempt in range(1, max_attempts + 1):
    ...
    def trigger_interaction(self, detect_signal, timeout_sec=60.0):
        mode = 'prefetch' if detect_signal else 'playback'
        self.get_logger().info(f'Triggering human interaction service ({mode})...')
        ...
        req.detect_signal = detect_signal
        ...
        try:
            ...
            if response and response.response_text:
                self.info(f"Received response from human interaction service: {response.response_text}")
                return response.response_text
            else:
                self.warn("Human interaction service returned an empty response.")
                return None
...
def main(args=None):
    ...
    try:
        ...
        for face_tuple in face_position_in_map_coordinates:
            ...
            prefetched_text = rc.prefetch_interaction()
            if prefetched_text:
                rc.info(f"Prefetched greeting for face [face: {face_tuple}]: {prefetched_text}")
            else:
                rc.warn(f"Could not prefetch greeting for face [face: {face_tuple}], will retry at playback.")
            ...
            while not rc.isTaskComplete():
                rc.info(f"Navigating to face detection [face: {face_tuple}]...")
                time.sleep(1)
            
            # TODO: start listening for voice:
            # TODO: voice commands...
            # play voice sound:
            response_text = rc.wait_for_interaction_completion()
            if response_text:
                rc.info(f"Finished interaction with human [face: {face_tuple}] using response: {response_text}")
            else:
                rc.warn(f"Skipping human [face: {face_tuple}] because playback could not be confirmed.")
